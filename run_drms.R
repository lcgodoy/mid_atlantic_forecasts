...
ctrl_file <- read_csv("control_file.csv") %>% 
  filter(id %in% c('v0.13','v0.39','v0.40','v0.58','v0.59'))
...
for(i in ctrl_file$id){
  ...
  if(make_plots==TRUE){
    ...
    fit_est_abund_tile <- abund_p_y_hat %>% 
      ggplot(aes(x=year, y=patch, fill=density_hat)) +
      geom_tile() +
      theme_bw() +
      scale_x_continuous(breaks=seq(0, 36, 4)) +
      scale_y_continuous(breaks=seq(1, np, 1)) +
    ...
    ggsave(fit_dat_abund_tile, filename=here(results_path,"observed_abundance_v_time_tileplot.png"), width=6)
    ggsave(fit_est_abund_tile, filename=here(results_path,"estimated_abundance_v_time_tileplot.png"), width=6)
    ...
    ggsave(proj_estimated_abundance_tile, filename=here(paste0("results/",i),"proj_estimated_abundance_v_time_tileplot.png"), scale=0.9, width=6)
    ggsave(proj_observed_abundance_tile, filename=here(paste0("results/",i),"proj_observed_abundance_v_time_tileplot.png"), scale=0.9, width=6)
    ...
    # 
    ggsave(observed_abundance_forecast, filename=here(paste0("results/",i), "abundance_obs_v_time_by_patch_proj.png"), dpi=300, width=8, height=6)
    ggsave(abundance_forecast, filename=here(paste0("results/",i), "abundance_true_v_time_by_patch_proj.png"), dpi=300, width=8, height=6)
...
}# close for loop for each model id
