...
```{r, fig.cap="Frequency distribution of summer flounder abundance in all hauls used in the analysis (testing and training data combined; *n* = 12,203)."}
...
```{r, fig.cap="Median density estimated by the DRM in the training dataset by patch and year (black line). Blue shading represents the 50%, 80%, and 95% credible intervals. Red points are the observed data."}
...
plot_out <- density_hat_thin %>% 
  ...
  labs(x="Year",y="Density", title = ctrl_file |> filter(id == k ) |> pull(name)) + 
  scale_fill_brewer() +
...
```{r, fig.cap="Posterior distributions of three important parameters from the four fitted DRMs. These parameters are *d*, the annual dispersal rate between adjacent patches; *Topt*, the optimal temperature estimated for whichever process (movement or recruitment or mortality) was estimated as temperature-dependent; and *width*, a parameter controlling how sensitive the modeled process was to temperature."}
out <- NULL

for(i in ctrl_file$id){
    results_path <- here('results',i)
    fixed_param_dat <- read_rds(file.path(here("results"),i,"fixed_params_averaged.rds"))
    fixed_param_dat$name <- ctrl_file[ctrl_file$id==i,]$name
    out <- rbind(out, fixed_param_dat)
}
