...
matrix[] simulate_population(int np, int ny_train, int n_ages, int n_lbins, int age_at_maturity, matrix sbt, real Topt, real width, real exp_yn, int T_dep_mortality, int T_dep_movement, matrix f_a_y, real m, real d, real beta_t, int T_dep_recruitment, int spawner_recruit_relationship, vector init_dep, real mean_recruits, real beta_rec, real sigma_r, vector raw, real r0, vector maturity_at_age, vector wt_at_age, real alpha, real h, real ssb0, vector d_at_age,matrix l_at_a_key, vector selectivity_at_bin, real beta_obs_int, real beta_obs, int number_quantiles, matrix init_n_at_age, int use_init_n_at_age){
    ...
    for (y in 2:ny_train){
      ...
      if (y == 2){ 
        ...
        rec_dev[y-1] =  alpha * rec_dev[y-2] +  sqrt(1 - pow(alpha,2)) *  sigma_r * raw[y]; 
      ...
      if(T_dep_movement==0){
        ...
        for(p in 1:np){
          for(a in 2:n_ages){
            ...
            if(p==1){
              n_at_age_hat[y,p,a] = n_at_age_hat[y-1,p, a-1] * surv[p,a-1,y-1] * (1-d_at_age[a-1]) + n_at_age_hat[y-1,p+1, a-1] * surv[p+1,a-1,y-1] * d_at_age[a-1];
            } // close patch 1 case 
              ...
              n_at_age_hat[y,p,a] = n_at_age_hat[y-1,p, a-1] * surv[p,a-1,y-1] * (1-2*d_at_age[a-1]) + n_at_age_hat[y-1,p-1, a-1] * surv[p-1,a-1,y-1] * d_at_age[a-1] + n_at_age_hat[y-1,p+1, a-1] * surv[p+1,a-1,y-1] * d_at_age[a-1];
      ...
      if(T_dep_movement==1){
        ...
        for (p in 1:np){
          ...
          n_at_age_hat[y,p, 2:n_ages] = n_at_age_hat[y-1,p,  1:(n_ages - 1)] .* to_row_vector(surv[p, 1:(n_ages - 1),y-1]);
...
transformed parameters{
  ...
  n_at_age_hat = simulate_population(np,  ny_train,  n_ages,  n_lbins,  age_at_maturity,  sbt,  Topt,  width,  exp_yn,  T_dep_mortality,  T_dep_movement,  f,  m,  d,  beta_t, T_dep_recruitment,  spawner_recruit_relationship,  init_dep,  mean_recruits,  beta_rec,sigma_r,  raw,  r0,  maturity_at_age,  wt_at_age,  alpha,  h,  ssb0,  d_at_age, l_at_a_key,  selectivity_at_bin,  beta_obs_int,  beta_obs,  number_quantiles, init_n_at_age, 0);
...
generated quantities {
  ...
  matrix[np, n_ages] n_at_age_proj[ny_proj+1]; // fix this
  matrix[np, n_lbins] n_at_length_proj[ny_proj+1];
  real density_obs_proj[np, ny_proj+1]; 
  real density_proj[np, ny_proj+1]; 
  ...
  if(run_forecast==1){
    ...
    n_at_age_proj = simulate_population(np,  ny_proj+1,  n_ages,  n_lbins,  age_at_maturity,  sbt_proj,  Topt,  width,  exp_yn,  T_dep_mortality,  T_dep_movement,  f_proj,  m,  d,  beta_t, T_dep_recruitment,  spawner_recruit_relationship,  init_dep,  mean_recruits,  beta_rec,sigma_r,  raw_proj,  r0,  maturity_at_age,  wt_at_age,  alpha,  h,  ssb0,  d_at_age, l_at_a_key,  selectivity_at_bin,  beta_obs_int,  beta_obs,  number_quantiles, (n_at_age_hat[ny_train,,]), 1);
